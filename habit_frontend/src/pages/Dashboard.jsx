...
// ─── Persistent today-mark per habit ─────────────────────────────────────────
// Stores in localStorage so navigating away doesn't reset the button
function getTodayKey(habitId) {
    const today = new Date().toISOString().split("T")[0];
    return `marked_${habitId}_${today}`;
}
function wasMarkedToday(habitId) {
    return localStorage.getItem(getTodayKey(habitId)) === "1";
}
function persistMarkToday(habitId) {
    localStorage.setItem(getTodayKey(habitId), "1");
}

// ─── Constellation goal stored per habit in localStorage ──────────────────────
...
function setStoredGoal(habitId, name) {
    if (name) localStorage.setItem(getGoalKey(habitId), name);
    else localStorage.removeItem(getGoalKey(habitId));
}
...
async function apiFetch(path, options = {}) {
    ...
    if (res.status === 401 || res.status === 403) {
        localStorage.removeItem("token"); window.location.href = "/"; return null;
    }
...
function GoalPicker({ onSelect, onClose, currentGoal }) {
    return (
        <div style={{ position: "fixed", inset: 0, background: "rgba(2,9,23,0.94)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, backdropFilter: "blur(10px)" }} onClick={onClose}>
            <div onClick={e => e.stopPropagation()} style={{ background: "#070f1e", border: "1px solid #1e293b", borderRadius: 22, padding: "30px 26px", maxWidth: 720, width: "92vw", maxHeight: "88vh", overflowY: "auto", animation: "celebrate 0.3s ease" }}>
                <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 22, fontWeight: 800, color: "#f1f5f9", marginBottom: 4 }}>Choose your constellation goal</div>
                <div style={{ fontSize: 13, color: "#475569", marginBottom: 24 }}>
                    You can see what each constellation looks like before committing. Stars light up one by one as your streak grows.
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 14 }}>
                    {CONSTELLATIONS.map(c => {
                        const W = 180, H = 100, p = 9;
                        const px = nx => p + (nx / 100) * (W - p * 2);
                        const py = ny => p + (ny / 100) * (H - p * 2);
                        const isCur = currentGoal === c.name;
                        return (
                            <button key={c.name} onClick={() => onSelect(c.name)} style={{ background: isCur ? "rgba(99,102,241,0.12)" : "#040d1a", border: `1.5px solid ${isCur ? "#6366f1" : "#1a2540"}`, borderRadius: 16, padding: "14px 13px 11px", cursor: "pointer", textAlign: "left", transition: "all 0.18s" }}
                            ...
                            >
                                {/* Full constellation preview — ALL stars visible (goal preview) */}
                                <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ borderRadius: 9, display: "block" }}>
                                    <rect width={W} height={H} fill="#020917" rx={9} />
                                    {c.lines.map(([a, b], i) => (
                                    ...
                                    {c.stars.map((s, i) => (
                                        <circle key={i} cx={px(s.x)} cy={py(s.y)} r={2} fill="#f0f9ff" fillOpacity={0.5} />
                                    ))}
                                </svg>
                                <div style={{ marginTop: 10 }}>
                                    ...
                                    <div style={{ fontSize: 10, color: "#334155", marginTop: 4, lineHeight: 1.4 }}>{c.myth.slice(0, 58)}…</div>
                                </div>
                                {isCur && <div style={{ marginTop: 8, fontSize: 10, color: "#6366f1", fontWeight: 700 }}>✓ Current goal</div>}
                            </button>
                        );
                    })}
                </div>
                <button onClick={onClose} style={{ marginTop: 22, padding: "9px 24px", background: "#1e293b", border: "none", borderRadius: 9, color: "#94a3b8", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
            </div>
        </div>
    );
}

// ─── Constellation hover map (shows current progress) ─────────────────────────
        ...
        <div style={{ position: "relative" }}>
            <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ display: "block", borderRadius: 10 }}>
                ...
                {Array.from({ length: 16 }).map((_, i) => (
                    <circle key={`a${i}`} cx={(((i * 137.5) % 100) / 100) * W} cy={(((i * 97.3) % 100) / 100) * H} r={0.5} fill="#f0f9ff" fillOpacity={0.06} />
                ))}
                {/* Lines — only lit between completed stars */}
                {lines.map(([a, b], i) => {
                    const ad = a < completedDays, bd = b < completedDays;
                    return <line key={i}
                                 x1={px(stars[a].x)} y1={py(stars[a].y)}
                                 x2={px(stars[b].x)} y2={py(stars[b].y)}
                                 stroke={ad && bd ? "#c4d9f5" : "#0f1f35"}
                                 strokeWidth={ad && bd ? 1.2 : 0.5}
                                 strokeOpacity={ad && bd ? 0.6 : 0.2}
                                 strokeDasharray={ad !== bd ? "3 3" : undefined} />;
                })}
                {/* ALL stars visible — completed ones glow */}
                {stars.map((s, i) => {
                    const done = i < completedDays;
                    const next = i === completedDays;
                    const hov  = hIdx === i;
                    const r = done ? 4.2 : next ? 2.8 : 1.8;
                    return (
                        <g key={i} onMouseEnter={() => setHIdx(i)} onMouseLeave={() => setHIdx(null)} style={{ cursor: "default" }}>
                            ...
                            {/* Dull locked stars always visible */}
                            <circle cx={px(s.x)} cy={py(s.y)} r={done ? 0 : 1.8}
                                    fill="#1e3a5f" fillOpacity={next ? 0.8 : 0.4} />
                            {/* Glow halo for done stars */}
                            {done && <circle cx={px(s.x)} cy={py(s.y)} r={hov ? 12 : 8} fill="#f0f9ff" fillOpacity={hov ? 0.12 : 0.05} />}
                            {/* Next-star pulse ring */}
                            {next && <circle cx={px(s.x)} cy={py(s.y)} r={7} fill="none" stroke="#60a5fa" strokeWidth={1} strokeOpacity={0.5} strokeDasharray="2 2" />}
                            {/* Main star */}
                ...
                {/* Progress text */}
                <text x={W - 7} y={H - 5} textAnchor="end" fontSize={9} fill="#1e3a5f" fontFamily="monospace">{completedDays}/{days}d · {Math.round(pct * 100)}%</text>
            </svg>
            {/* Per-star tooltip */}
            {hIdx !== null && (
                <div style={{ position: "absolute", top: 4, left: `${Math.min(Math.max((stars[hIdx].x / 100) * W, 60), W - 60)}px`, transform: "translateX(-50%)", background: "rgba(4,13,26,0.97)", border: "1px solid #1e3a5f", borderRadius: 7, padding: "4px 10px", fontSize: 11, fontWeight: 700, color: "#f1f5f9", whiteSpace: "nowrap", pointerEvents: "none", zIndex: 20 }}>
                    Day {hIdx + 1}
                    {hIdx < completedDays ? <span style={{ color: "#4ade80", marginLeft: 5 }}>✓ done</span>
                        : hIdx === completedDays ? <span style={{ color: "#60a5fa", marginLeft: 5 }}>← next</span>
                            : <span style={{ color: "#334155", marginLeft: 5 }}>locked</span>}
                </div>
            )}
        </div>
    );
...
// ─── Hover popup that floats above/below the card ────────────────────────────
function ConstellationPopup({ habit, goalConstellation, streak, onChangeGoal }) {
                ...
                {goalConstellation ? (
                    <>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                            <div>
                                <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 14, color: "#f1f5f9" }}>{goalConstellation.name}</span>
                                <span style={{ fontSize: 11, color: "#4ade80", fontWeight: 600, marginLeft: 8 }}>{goalConstellation.days}d goal</span>
                                <span style={{ fontSize: 11, color: "#6366f1", marginLeft: 6 }}>day {streak} of {goalConstellation.days}</span>
                            </div>
                        ...
                        {pct >= 1 && (
                            <div style={{ marginTop: 8, textAlign: "center", fontSize: 12, color: "#4ade80", fontWeight: 700 }}>🏆 Constellation complete! Choose your next goal →</div>
                        )}
                    </>
                ) : (
                    <div style={{ textAlign: "center", padding: "12px 0" }}>
                        ...
                        <div style={{ fontSize: 12, color: "#475569", marginBottom: 14 }}>Pick a target and watch stars connect as you build your streak.</div>
                        <button onClick={e => { e.stopPropagation(); onChangeGoal(); }} style={{ padding: "9px 22px", background: "linear-gradient(90deg,#6366f1,#8b5cf6)", border: "none", borderRadius: 9, color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "'Syne',sans-serif" }}>Pick a Constellation →</button>
                    </div>
                )}
            </div>
        </div>
...
function HabitCalendar({ habit, calendar, onClose }) {
    ...
    const calSet = new Set(normalizeDates(calendar));
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(2,9,23,0.88)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 200, backdropFilter: "blur(8px)" }} onClick={onClose}>
            <div onClick={e => e.stopPropagation()} style={{ background: "#070f1e", border: "1px solid #1e293b", borderRadius: 20, padding: "22px 22px 18px", minWidth: 306, animation: "celebrate 0.3s ease" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                    <div>
                        <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 14, color: "#f1f5f9" }}>{habit.name}</div>
                        <div style={{ fontSize: 10, color: "#475569", marginTop: 1 }}>{monthName} {year}</div>
                    </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(2,9,23,0.97)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", zIndex: 300 }}>
            <div style={{ animation: "float 3s ease infinite", textAlign: "center" }}>
                ...
                <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} style={{ borderRadius: 14 }}>
                    <defs>
                        <radialGradient id="cb6" cx="50%" cy="50%" r="50%"><stop offset="0%" stopColor="#0d1b35" /><stop offset="100%" stopColor="#020917" /></radialGradient>
                        <filter id="gl6"><feGaussianBlur stdDeviation="3.5" result="b" /><feMerge><feMergeNode in="b" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
                    </defs>
                    <rect width={W} height={H} fill="url(#cb6)" rx={14} />
                    ...
                    {stars.map((s, i) => <g key={i}><circle cx={px(s.x)} cy={py(s.y)} r={8} fill="#f0f9ff" fillOpacity={0.06} /><circle cx={px(s.x)} cy={py(s.y)} r={4} fill="#f0f9ff" filter="url(#gl6)" /></g>)}
                </svg>
            </div>
            <button onClick={onDismiss} style={{ marginTop: 26, padding: "12px 38px", background: "#4ade80", color: "#04080f", border: "none", borderRadius: 12, fontFamily: "'Syne',sans-serif", fontSize: 15, fontWeight: 800, cursor: "pointer" }}>
                Choose Next Target →
            </button>
        </div>
    );
...
function ProfileDropdown({ onLogout }) {
    ...
    const saveName = async () => {
        ...
        if (u) { setUser(u); setMsg("Saved! ✓"); setTimeout(() => setMsg(""), 2000); }
    ...
    return (
        <div ref={ref} style={{ position: "relative" }}>
            ...
            {open && (
                <div style={{ position: "absolute", right: 0, top: "calc(100% + 10px)", background: "#0a1628", border: "1px solid #1e293b", borderRadius: 16, minWidth: 240, padding: 8, boxShadow: "0 16px 40px rgba(0,0,0,0.7)", zIndex: 100 }}>
                    <div style={{ padding: "11px 12px 12px", borderBottom: "1px solid #1e293b", marginBottom: 6 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: editing ? 10 : 0 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontWeight: 700, fontSize: 13, color: "#f1f5f9", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user?.name || "—"}</div>
                        ...
                        {editing ? (
                            <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                                ...
                                <div style={{ display: "flex", gap: 7 }}>
                                    <button onClick={saveName} disabled={saving} style={{ ...SMB("#4ade80","#04080f"), padding: "5px 12px", fontSize: 11 }}>{saving ? "…" : "Save"}</button>
                                    <button onClick={() => setEditing(false)} style={{ ...SMB("#1e293b","#94a3b8"), padding: "5px 12px", fontSize: 11 }}>Cancel</button>
                                </div>
                                {msg && <div style={{ fontSize: 10, color: "#4ade80" }}>{msg}</div>}
                            </div>
                        ) : (
                            <button onClick={() => setEditing(true)} style={{ marginTop: 7, width: "100%", padding: "5px 9px", background: "#0f172a", border: "1px solid #1e293b", borderRadius: 7, color: "#6366f1", fontSize: 11, fontWeight: 600, cursor: "pointer", textAlign: "left" }}>✎ Edit name</button>
                        )}
                    </div>
                    <button onClick={() => window.location.href = "/analytics"}
                            onMouseEnter={e => e.currentTarget.style.background = "rgba(99,102,241,0.08)"}
                            onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                            style={{ width: "100%", padding: "8px 12px", background: "transparent", border: "none", borderRadius: 7, color: "#6366f1", fontSize: 12, fontWeight: 600, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 7 }}>
                        ◈ Analytics
                    </button>
                    <button onClick={onLogout}
                            onMouseEnter={e => e.currentTarget.style.background = "rgba(248,113,113,0.08)"}
                            onMouseLeave={e => e.currentTarget.style.background = "transparent"}
                            style={{ width: "100%", padding: "8px 12px", background: "transparent", border: "none", borderRadius: 7, color: "#f87171", fontSize: 12, fontWeight: 600, cursor: "pointer", textAlign: "left", display: "flex", alignItems: "center", gap: 7 }}>
                        ↩ Logout
                    </button>
                </div>
            )}
...
function HabitCard({ habit, onDelete, onUpdate, onCelebrate }) {
    // ✅ Persistent mark state — survives navigation
    ...
    // Goal constellation stored in localStorage
    ...
    const loadStats = useCallback(async () => {
        ...
        if (s) {
            ...
            // Check if goal constellation just completed
            if (goalConstellation && prevStreakRef.current !== null && prevStreakRef.current < goalConstellation.days && newStreak >= goalConstellation.days) {
                onCelebrate(goalConstellation);
            }
        ...
        if (cal) {
            const arr = normalizeDates(Array.isArray(cal) ? cal : []);
            setCalendar(arr);
            // ✅ Sync localStorage mark with server
            const today = new Date().toISOString().split("T")[0];
            if (arr.includes(today)) persistMarkToday(habit.id);
    ...
    // ✅ Mark is one-way — cannot unmark
    const handleMark = async () => {
        if (marked) return;
        setLoading(true);
        await apiFetch(`/api/logs/${habit.id}/mark`, { method: "POST" });
        persistMarkToday(habit.id);
        setMarked(true);
        await loadStats();
        setLoading(false);
    };
    ...
    const handleGoalSelect = (name) => {
        setGoalName(name);
        setStoredGoal(habit.id, name);
        setShowGoalPicker(false);
    };

    return (
        <>
            {showCalendar && <HabitCalendar habit={habit} calendar={calendar} onClose={() => setShowCalendar(false)} />}
            {showGoalPicker && <GoalPicker currentGoal={goalName} onSelect={handleGoalSelect} onClose={() => setShowGoalPicker(false)} />}

            <div ref={cardRef} style={{ position: "relative", background: "linear-gradient(160deg,#070f1e,#0a1628)", border: marked ? "1.5px solid rgba(240,249,255,0.2)" : "1.5px solid #0f1a2e", borderRadius: 20, padding: "18px", display: "flex", flexDirection: "column", gap: 12, transition: "border 0.3s, box-shadow 0.3s", boxShadow: marked ? "0 0 22px rgba(240,249,255,0.04)" : "none", animation: "fadeIn 0.3s ease" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    {!editing && (
                        <div style={{ display: "flex", gap: 4, flexShrink: 0 }}>
                            <button onClick={() => setShowCalendar(true)} title="Calendar" style={ICN("#0f172a","#475569")}>📅</button>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: 2 }}>
                        {Array.from({ length: 7 }).map((_, i) => <div key={i} style={{ width: 6, height: 14, borderRadius: 2, background: i < weekly ? "#f0f9ff" : "#0f1a2e" }} />)}
                    </div>
                </div>

                {/* Constellation goal strip */}
                <div style={{ position: "relative" }}>
                    <div onClick={() => setShowPopup(p => !p)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "7px 10px", background: "#040d1a", borderRadius: 8, border: `1px solid ${showPopup ? "#1e3a5f" : "#0f1a2e"}`, cursor: "pointer", transition: "border 0.2s" }}>
                        <span style={{ fontSize: 11 }}>
                            {goalName
                                ? <><span style={{ color: "#6366f1", fontWeight: 600 }}>✦ {goalName}</span><span style={{ color: "#1e3a5f" }}> · hover to see stars</span></>
                                : <span style={{ color: "#334155", fontWeight: 600 }}>✦ Set your constellation goal</span>
                            }
                        </span>
                    ...
                    {showPopup && (
                        <ConstellationPopup
                            habit={habit}
                            goalConstellation={goalConstellation}
                            streak={streak}
                            onChangeGoal={() => { setShowPopup(false); setShowGoalPicker(true); }}
                        />
                    )}
                </div>

                {/* ✅ Mark Done — permanent once clicked */}
                <button onClick={handleMark} disabled={marked || loading} style={{ padding: "9px 0", borderRadius: 9, border: "none", cursor: marked ? "default" : loading ? "not-allowed" : "pointer", fontWeight: 700, fontSize: 13, background: marked ? "linear-gradient(90deg,#f0f9ff,#e2e8f0)" : "linear-gradient(90deg,#0f1a2e,#1a2b42)", color: marked ? "#04080f" : "#475569", transition: "all 0.3s", opacity: loading ? 0.6 : 1 }}>
                    {loading ? "…" : marked ? "✦ Shining Today ✓" : "Mark as Done"}
                </button>

                {/* ✅ Save Streak — permanent once clicked */}
                <button onClick={handleSaveStreak} disabled={marked || loading} style={{ padding: "8px 0", borderRadius: 9, border: marked ? "1.5px solid #22c55e" : "1.5px dashed #1e3a5f", background: "transparent", cursor: marked ? "default" : "pointer", fontWeight: 600, fontSize: 12, color: marked ? "#22c55e" : "#1e3a5f", transition: "all 0.2s", opacity: marked ? 0.7 : 1 }}>
        ...
        <>
            ...
            <div style={{ minHeight: "100vh", background: "#020917", color: "#f1f5f9", fontFamily: "'DM Sans','Segoe UI',sans-serif", paddingBottom: 60 }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 32px", borderBottom: "1px solid #070f1e", background: "rgba(2,9,23,0.96)", position: "sticky", top: 0, zIndex: 40, backdropFilter: "blur(12px)" }}>
                    <div>
                        <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 18, fontWeight: 800 }}><span style={{ color: "#f0f9ff", opacity: 0.4 }}>✦</span> HabitTracker</div>
                        <div style={{ fontSize: 10, color: "#1e3a5f", marginTop: 1 }}>Map your stars · build your constellation</div>
                    </div>
                ...
                <div style={{ maxWidth: 960, margin: "0 auto", padding: "28px 16px" }}>
                    {habits.length > 0 && (
                        <div style={{ marginBottom: 20 }}>
                            <div style={{ background: "#070f1e", border: "1px solid #0f1a2e", borderRadius: 10, padding: "12px 20px", display: "inline-block" }}>
                    ...
                    {/* ✅ Add Habit → goes to SelectHabits page in "add" mode */}
                    <div style={{ marginBottom: 24 }}>
                        <button onClick={() => window.location.href = "/select-habits?mode=add"} style={{ background: "linear-gradient(90deg,#6366f1,#8b5cf6)", color: "#fff", border: "none", borderRadius: 10, padding: "10px 22px", fontWeight: 700, fontSize: 13, cursor: "pointer", fontFamily: "'Syne',sans-serif", boxShadow: "0 4px 14px rgba(99,102,241,0.3)" }}>
                            + New Habit
                        </button>
                    </div>

                    {loading ? (
                        <div style={{ textAlign: "center", color: "#1e3a5f", paddingTop: 70 }}>Mapping the stars…</div>
                    ) : habits.length === 0 ? (
                        <div style={{ textAlign: "center", paddingTop: 70 }}>
                            <div style={{ fontSize: 48, marginBottom: 12 }}>✦</div>
                            <div style={{ fontFamily: "'Syne',sans-serif", fontSize: 16, fontWeight: 700, color: "#1e3a5f", marginBottom: 4 }}>No habits yet</div>
                            <div style={{ fontSize: 12, color: "#0f1a2e" }}>Click "+ New Habit" to get started</div>
                        </div>
                    ) : (
